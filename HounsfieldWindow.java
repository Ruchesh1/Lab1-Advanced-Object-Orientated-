...
public class HounsfieldWindow {
	...
	public HounsfieldWindow(int level, int width) {
		if(level < Hounsfield.MIN_VALUE || level > Hounsfield.MAX_VALUE) {
			throw new IllegalArgumentException("level is outsides the range of Hounsfield.");
		}
		if(width < 1) {
	...
	public int setLevel(int level) {
		if(level < Hounsfield.MIN_VALUE || level > Hounsfield.MAX_VALUE) {
			throw new IllegalArgumentException("level is outside the range of Hounsfield");
		}
	...
	public double map(Hounsfield h) {
		double lo = (double) this.level - ((double) this.width / 2);
		double hi = (double) this.level + ((double) this.width / 2);
		if(h.get() > hi) {
			return 1;
		}
		if(h.get() < lo) {
		...
		return ((double) h.get() - lo)/this.width;
	}
